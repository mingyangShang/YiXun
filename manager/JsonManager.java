...
public class JsonManager {
	private static final String FLAG="flag",PHONE="phone",PASSWORD="password",
			...
			CODE="code",MSG="msg",MAN="man",WOMAN="woman",
	...
	//注册操作,加不加性别再说
	public static JSONObject register(String phone,String name,String pass,String sex){
		JSONObject person = new JSONObject();
		try{
			person.put(FLAG, HTTP_SUFFIX);
			person.put(PHONE,phone);
...
//			contact.discussions="";
	...
	//更新签名操作,有没有性别再说
	public static JSONObject updateSign(String oldphone,String sign,String sex){
		JSONObject person = new JSONObject();
		try{
			person.put(FLAG, HTTP_SUFFIX);
...
//					f.weixin=obj.getString(WEIXIN);
			...
			//发送消息
			public static JSONObject SendMessage(String myphone,String phone,String msg,String time){
			...
			//想服务器发送通知
			public static String sendNotice(String myphone,String[] phones,String msg,String time){
				JSONObject json=new JSONObject();
				String ran8=UUID.randomUUID().toString().substring(0, 8)+"]";
				try{
					...
					json.put("msg", msg);
					json.put("from_phone", myphone);
					json.put("to_phone", phones.toString());
			...
			public static String replyNotice(String myphone,String phone,String msg,String time_send,String time_reply){
				...
				try{
					...
					json.put("msg", msg);
					json.put("from_phone", myphone);
					json.put("to_phone", phone.toString());
